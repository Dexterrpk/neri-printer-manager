...
class SafeEnhancedWindow(EnhancedWindow):
    ...
    def _tools(self):
        ...
        if layout is None:
            return page
        usb_title = QLabel("Impressoras USB")
        usb_title.setObjectName("title")
        layout.insertWidget(2, usb_title)
        self.usb_status = QLabel("Conecte a impressora USB e clique em Procurar USB.")
        self.usb_status.setObjectName("muted")
        layout.insertWidget(3, self.usb_status)
        self.usb_table = self._table(
            ["Impressora USB", "Fabricante", "Modelo", "Driver recomendado"]
        )
        layout.insertWidget(4, self.usb_table)
        row = QHBoxLayout()
        row.addWidget(self._button("Procurar USB", self.discover_usb, True))
        row.addWidget(self._button("Instalar USB selecionada", self.install_usb))
        row.addStretch()
        layout.insertLayout(5, row)
        return page
    ...
    def _sharing(self):
        ...
        row.addWidget(
            self._button(
                "Compartilhar impressora selecionada",
                self.share_selected_printer,
                True,
            )
        )
        row.addStretch()
        ...
        note = QLabel(
            "Selecione primeiro a fila em Minhas impressoras. O programa habilita o "
            "compartilhamento CUPS e prepara o Samba para acesso autenticado."
        )
    ...
    def discover_usb(self):
        self.usb_status.setText(
            "Procurando dispositivos USB e comparando drivers instalados..."
        )
    ...
    def _show_usb(self, items):
        ...
        self.usb_status.setText(
            ...
            if items
            else "Nenhuma impressora USB detectada pelo CUPS."
        )
        if items:
    ...
    def install_usb(self):
        ...
        if row < 0 or row >= len(self.usb_items):
            QMessageBox.information(self, "Selecione", "Escolha uma impressora USB.")
            return
        item = self.usb_items[row]
        self._run(
            lambda: UsbPrinterService().install(item),
            lambda result: (
                QMessageBox.information(self, "USB instalada", result),
    ...
    def share_selected_printer(self):
        ...
        if not name:
            return
        self._run(
            lambda: UsbPrinterService().share(name),
            lambda text: (
                QMessageBox.information(self, "Compartilhamento", text),
